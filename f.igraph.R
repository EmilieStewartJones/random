# Function that takes an input table with motherinst, inst 
f.igraph <- function(table){
  ...
  undescribedmothers = na.omit(data.frame(table$motherinst[!(table$motherinst %in% table$inst)],stringsAsFactors = F))  # these are the mothers that are referred to in the edgelist but have no inst
