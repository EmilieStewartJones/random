...
f.subset <- function(con, calvingyr = NULL, calvingloc = NULL, wk_num = NULL, size = NULL, loc_x = NULL, loc_y = NULL) {
  ...
  # Prepare DB queries
    # Prepare calving year query
      ...
      else yr_query <- paste0(" calvingyr = ANY (SELECT calvingyr WHERE calvingyr = '", 
    ...
    # Prepare calvingloc query
      ...
      else loc_query <- paste0(" calvingloc = ANY (SELECT calvingloc WHERE calvingloc = '", 
    ...
    # Prepare size query
      ...
      else size_query <- paste0(" area >= ", size[1], " AND area <= ", size[2] )
      
    # Prepare loc_x query
      if (is.null(loc_x)) {
        loc_x_query <- NULL
      }
      else loc_x_query <- paste0(" lon >= ", loc_x[1], " AND lon <= ", loc_x[2] )
      
    # Prepare loc_y query
      if (is.null(loc_y)) {
        loc_y_query <- NULL
      }
      else loc_y_query <- paste0(" lat >= ", loc_y[1], " AND lat <= ", loc_y[2] )
      
    # Prepare total subquery
      if (is.null(calvingyr) & is.null(calvingloc) & is.null(size) & is.null(loc_x) & is.null(loc_y)){
        sub_query <- NULL
      }
      else sub_query <- paste0(" WHERE", paste0(c(yr_query, loc_query, size_query, loc_x_query, loc_y_query),
                                                collapse = " AND "))
    ...
    # Query from database
      query <- paste0("SELECT *, ST_AsText(CI2D3.geometry) as geom1, ",
                      ...
                      "as wk_num FROM CI2D3", sub_query) 
